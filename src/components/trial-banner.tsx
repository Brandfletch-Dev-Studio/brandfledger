...
export function TrialBanner() {
  ...
  async function loadTrial() {
    try {
      ...
      if (status === "trial" && trialEndsAt) {
        daysLeft = Math.ceil((new Date(trialEndsAt).getTime() - Date.now()) / 86400000);
      } else if (status === "active" && subscriptionEndsAt) {
        daysLeft = Math.ceil((new Date(subscriptionEndsAt).getTime() - Date.now()) / 86400000);
      }
  ...
  // Only show banner if trial is running low or expired
  if (trial.status === "active" && trial.daysLeft > 7) return null;
    ...
    <div className={cn(
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <span className="truncate">
          {isExpired ? "Your trial has expired." : `${trial.daysLeft} day${trial.daysLeft !== 1 ? "s" : ""} left in your trial.`}
        </span>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <Button size="sm" variant="secondary" className="h-7 text-xs gap-1" onClick={() => router.push("/subscription")}>
